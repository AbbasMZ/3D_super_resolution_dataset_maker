...
parser.add_argument('-on', '--output_name', default=None, type=str, dest='output_name')
...
if args.dimension == '2':
    for index in range(fdata.shape[args.axis_2D]):
        ...
        if args.output_name is not None:
            save_path = args.output_path_HR + args.output_name + '_' + str(index) + args.output_format
        else:
            save_path = args.output_path_HR + args.input_name + '_' + str(index) + args.output_format
...
if args.dimension == '3':
    if args.output_name is not None:
        save_path = args.output_path_LR + args.output_name + args.output_format
    else:
        save_path = args.output_path_LR + args.input_name + args.output_format
    ...
    for index in range(LR_img_real.shape[args.axis_2D]):
        ...
        # save image as HR
        if args.output_name is not None:
            save_path = args.output_path_LR + args.output_name + '_' + str(index) + args.output_format
        else:
            save_path = args.output_path_LR + args.input_name + '_' + str(index) + args.output_format
